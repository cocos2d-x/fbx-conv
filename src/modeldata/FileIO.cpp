...
void write(float value, FILE *file) { fwrite(&value, sizeof(float), 1, file); }
...
void writeZero(FILE *file) { write((unsigned int)0, file); }
...
void skipUint(FILE *file) { fseek(file, sizeof(unsigned int), SEEK_CUR); }
