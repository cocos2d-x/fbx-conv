...
struct Mesh final : public json::ConstSerializable {
    ...
    Mesh(const Mesh &copyFrom) {
        ...
        vertices.insert(vertices.end(), copyFrom.vertices.begin(),
                        copyFrom.vertices.end());
        for (std::vector<MeshPart *>::const_iterator itr =
                 copyFrom.parts.begin();
             itr != copyFrom.parts.end(); ++itr)
    ...
    void clear() {
        ...
        for (std::vector<MeshPart *>::iterator itr = parts.begin();
             itr != parts.end(); ++itr)
            delete (*itr);
        parts.clear();
        pnList.clear();
    ...
    inline unsigned int indexCount() {
        ...
        for (std::vector<MeshPart *>::const_iterator itr = parts.begin();
             itr != parts.end(); ++itr)
    ...
    inline unsigned int add(const float *vertex) {
        ...
        for (unsigned int i = 0; i < n; i++)
            if ((hashes[i] == hash) &&
                compare(&vertices[i * vertexSize], vertex, vertexSize))
                return i;
    ...
    inline void calcNormal() {
        ...
        for (auto iter = pnList.begin(); iter != pnList.end(); iter++) {
    ...
    inline void calcAABB() {
        ...
        for (unsigned int i = 0; i < parts.size(); ++i) {
            ...
            for (auto iter : meshPart->indices) {
                ...
                // update aabb min
                meshPart->aabb[0] = position[0] < meshPart->aabb[0]
                                        ? position[0]
                                        : meshPart->aabb[0];
                meshPart->aabb[1] = position[1] < meshPart->aabb[1]
                                        ? position[1]
                                        : meshPart->aabb[1];
                meshPart->aabb[2] = position[2] < meshPart->aabb[2]
                                        ? position[2]
                                        : meshPart->aabb[2];
                // update aabb max
                meshPart->aabb[3] = position[0] > meshPart->aabb[3]
                                        ? position[0]
                                        : meshPart->aabb[3];
                meshPart->aabb[4] = position[1] > meshPart->aabb[4]
                                        ? position[1]
                                        : meshPart->aabb[4];
                meshPart->aabb[5] = position[2] > meshPart->aabb[5]
                                        ? position[2]
                                        : meshPart->aabb[5];
            }
        }
