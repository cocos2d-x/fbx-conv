...
namespace fbxconv {
namespace modeldata {
template <typename T, size_t n = 1> struct OptionalValue {
    bool valid;
    T value[n];
...
template <typename T> struct OptionalValue<T, 1> {
    bool valid;
    T value;
...
struct Material final : public json::ConstSerializable {
    struct Texture final : public json::ConstSerializable {
        enum Usage {
            Unknown = 0,
            None = 1,
            Diffuse = 2,
    ...
    Material()
        : source(0), diffuse(), ambient(), emissive(), specular(), shininess(),
          opacity() {}

    Material(const Material &rhs)
        : source(rhs.source), id(rhs.id), diffuse(rhs.diffuse),
          ambient(rhs.ambient), emissive(rhs.emissive), specular(rhs.specular),
          shininess(rhs.shininess), opacity(rhs.opacity) {}
    ...
    Texture *getTexture(const char *id) const {
        for (std::vector<Texture *>::const_iterator itr = textures.begin();
             itr != textures.end(); ++itr)
