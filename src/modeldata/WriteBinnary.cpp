...
namespace modeldata {
static const char *
getTextureUseString(const Material::Texture::Usage &textureUse) {
    switch (textureUse) {
...
static const char *
getWrapModeUseString(const FbxFileTexture::EWrapMode &textureUse) {
    switch (textureUse) {
    ...
    if (parts.size() > 0) {
        for (unsigned int i = 0; i < parts.size(); i++) {
            ...
            if (nodepart) {
                ...
                write(size, file);
                for (auto itr = nodepart->bones.begin();
                     itr != nodepart->bones.end(); itr++) {
                    // write name
                ...
                write(size, file);
                for (auto itr = nodepart->uvMapping.begin();
                     itr != nodepart->uvMapping.end(); itr++) {
                    unsigned int size = itr->size();
                    write(size, file);
                    // TextureIndex
                    for (auto tt = (*itr).begin(); tt != (*itr).end(); ++tt) {
                        unsigned int index =
                            nodepart->material->getTextureIndex(*tt);
                        write(index, file);
                    }
                }
            }
...
void Animation::writeBinary(FILE *file) {
    ...
    for (auto itr = nodeAnimations.begin(); itr != nodeAnimations.end();
         itr++) {
        ...
        for (auto itr1 = nodeanim->keyframes.begin();
             itr1 != nodeanim->keyframes.end(); itr1++) {
