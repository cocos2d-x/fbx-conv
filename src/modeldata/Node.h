...
struct Node final : public json::ConstSerializable {
    struct {
        float translation[3];
        float rotation[4];
        float scale[3];
    ...
    Node(const Node &copyFrom) {
        ...
        for (std::vector<NodePart *>::const_iterator itr =
                 copyFrom.parts.begin();
             itr != copyFrom.parts.end(); ++itr)
            parts.push_back(new NodePart(**itr));
        for (std::vector<Node *>::const_iterator itr =
                 copyFrom.children.begin();
             itr != copyFrom.children.end(); ++itr)
            children.push_back(new Node(**itr));
        _skeleton = copyFrom._skeleton;
    }

    ~Node() {
        for (std::vector<NodePart *>::iterator itr = parts.begin();
             itr != parts.end(); ++itr)
            delete (*itr);
        for (std::vector<Node *>::iterator itr = children.begin();
             itr != children.end(); ++itr)
            delete (*itr);
    }

    Node *getChild(const char *id) const {
        for (std::vector<Node *>::const_iterator itr = children.begin();
             itr != children.end(); ++itr) {
    ...
    void loadBoneNames(std::list<std::string> &bonenames) {
        for (size_t i = 0; i < parts.size(); i++) {
            ...
            for (size_t j = 0; j < parts[i]->bones.size(); j++) {
    ...
    bool checkIsBone(const std::string &name,
        ...
        std::list<std::string>::iterator iter =
            std::find(bonenames.begin(), bonenames.end(), name);
    ...
    void setSkeleton(bool skeleton) { _skeleton = skeleton; }
};
} // namespace modeldata
} // namespace fbxconv
