...
struct NodePart final : public json::ConstSerializable {
    ...
    NodePart(const NodePart &copyFrom)
        : meshPart(copyFrom.meshPart), material(copyFrom.material) {
        ...
        for (unsigned int i = 0; i < uvMapping.size(); i++)
            uvMapping[i].insert(uvMapping[i].begin(),
                                copyFrom.uvMapping[i].begin(),
                                copyFrom.uvMapping[i].end());
    }
