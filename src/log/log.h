...
class Log {
  public:
    ...
    Log(LogMessages *const &messages, const int &filter = -1)
        : messages(messages), filter(filter) {}

    virtual ~Log() { delete messages; }

    const char *msg(int code) { return (*messages)[code]; }
