...
class JSONWriter : public BaseJSONWriter {
  public:
    ...
    JSONWriter(std::ostream &stream)
        : BaseJSONWriter(), newline("\r\n"), indent("\t"), openObject("{"),
          closeObject("}"), openArray("["), closeArray("]"), keySeparator(": "),
          valueSeparator(", "), stream(stream), indentCount(0) {}
  ...
  protected:
    ...
    virtual void writeNull() { stream << "null"; }
